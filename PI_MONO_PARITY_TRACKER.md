...
- `DONE` headless-safe skills/extensions parity
  ...
  - headless model-select and user-bash hooks cover model changes and direct bash interception
...
- `DONE` extension runtime parity for headless-safe command flows
  ...
  - `pigo` now provides executable extension command/tool registration, extension flags/statuses, resource discovery hooks, lifecycle events, cancellable lifecycle hooks, input/context transform hooks, provider request/response hooks, tool call/result hooks, before-agent-start hooks, model-select hooks, and user-bash hooks for headless sessions
...
- `DONE` `codingagent`: model-select and user-bash hooks for headless model changes and direct bash execution
...
- `DONE` `codingagent`: headless model-select/user-bash hooks cover model change observation and direct bash replacement
